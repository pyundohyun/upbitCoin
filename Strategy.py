...
class Strategy:
    ...
    def colculrate_rsi(self,ohlc: pandas.DataFrame, period: int = 14): 
        ...
        #log = Log().initLogger()
        #log.debug(pandas.Series(100 - (100/(1 + RS)), name = "RSI"))
    ...
    async def get_bigShort_coinList(self,coinName):
        ...
        try:
            #어제 오늘만 보고 전일대비로 체크 
            df = pyupbit.get_ohlcv(coinName,"day",count=2)
            ...
            log.debug(data_dict) # 받아오는 데이터 확인(해당 사이트가면 자세히 정리해서 안내하고 있습니다.)
            log.debug(data_dict['acc_trade_volume_24h'])
    ...
    async def goFindCoin(self,coinName):
        ...
        try:
            ...
            #print("[[[[[[[[[[  코인 서칭 중.......편도]]]]]]]]]]]]]"+ str(curTime))

            

            # 조건식에 걸리면 매수

            # 얘가 있으면 젤 급등 코인은 서칭되지만, 나머지를 못보니까
            # 일단 주석하고, 필요하면 다시 풀고, if문떙기면됨 
            ...
            log.debug("급등코인>>>"+coinName)
            log.debug("적정가>>>"+str(basicPriceValue))
            log.debug("1분전>>>"+str(before_coin_RIS[-1]))
            log.debug("2분전>>>"+str(before_coin_RIS[-2]))
            log.debug("3분전>>>"+str(before_coin_RIS[-3]))
            log.debug("4분전>>>"+str(before_coin_RIS[-4]))
            log.debug("5분전>>>"+str(before_coin_RIS[-5]))
            log.debug("6분전>>>"+str(before_coin_RIS[-6]))
        ...
        for items in myInfo:
            ...
            if(hastickers == coinNm):
                ...
                try:
                    ...
                    log.debug("[[[[[[[[[[[[ 보유 코인 정보]]]]]]]]]]]]")
                    log.debug("myWallet>>>"+str(myWallet))
                    ...
                    # if(chuMadPer) <= -0.5:
                    #     log.debug("-0.5 하락 추매 >>>>>>"+str(curPrice))
                    #     #시장가로 주문
                    #     CoinEvent().buyAndGazzza(coinName,"bid",orderVolumn,self.get_order_coin_price(),"price")
                    ...
                    if coinProfit > basicSellPercent:
                        ...
                        log.debug("추매전 이평선 확인!!")
                        log.debug("curPrice>>>"+str(curPrice))
                        log.debug("MA5>>>"+str(MA5))
                        log.debug("MA14>>>"+str(MA14))
                        log.debug("MA14>>>"+str(CoinEvent.get_pre_RIS_val(self,coinNm,"minute1",-1,-2)))
