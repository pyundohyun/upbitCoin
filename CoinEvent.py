...
class CoinEvent:
    ...
    def get_myProfitInfo(self,items):
        ...
        #  print("get_myProfitInfo curPrice>>>>"+str(curPrice))
        #  print("get_myProfitInfo buyPrice>>>>"+str(buyPrice))
        #  print("get_myProfitInfo profit>>>>"+str(profit))
        #  print("get_myProfitInfo profitPercent>>>>"+str(profitPercent))
    ...
    def get_diff_vol(self, coinName,interval,count):
        ...
        log = Log().initLogger()
        log.debug('거래량 확인중 .. 거름망 통과 서칭 코인>>>'+coinName)
        #log.debug('편도>>>'+str(diffVolume))
        log.debug("차이>>"+str(diffVolume.iloc[-1] + diffVolume.iloc[-2]))

        # print('서칭 코인>>>'+coinName)
        # print('편도>>>'+str(diffVolume))
    ...
    def get_pre_RIS_val(self,coinName,minuteType,index,index2):
        ...
        #5분봉기준 rsi 차이
        # print("분봉차이값-------------------------")
        # print(before_coin_RIS[-1] - before_coin_RIS[-2])
    ...
    def getMyPaymentList(self):
        try:
            ...
            for i, order in enumerate(_order_info_all):
                    ...
                    log.debug('[[[[[[Error]]]]]] Order_info_all Error>>>'+str(Err))
